...
> <b>프로젝트 목적 <b/>
> 1. 비대면 학기의 강의, 과제, 퀴즈 기한 홍수로 인한 결석, 미제출 방지 
> 2. 넓은 학교 부지 및 많은 건물로 인한 혼동 방지 
> 3. 학교앱에 없는 장학, 학식 실시간 알람 및 부서 전화번호부 제공
  </br>
  
> <b> 성과 <b/>
> - 강의 기한 긴급 알람을 통해 비대면 수강 편의성 극대화
> - 부서명 검색 ⇒ 카카오 맵 건물 마크 ⇒ 걸어서/뛰어서 걸리는 시간 표시로 학교 내 이동 효율성 증대
> - 자동 장학, 학식 알람을 통해 중요 정보를 놓치는 불편함 제거
> - 현재 시각 기온에 따른 추천 복장을 통해 등교 복장 고민 해소
...
<img src="https://user-images.githubusercontent.com/82270774/147848588-2f3fc9ba-e7df-41eb-bc11-dc58b6b772a9.png" width="23%"/>
<img src="https://user-images.githubusercontent.com/82270774/147848713-ad8e4903-57af-40ac-b324-da08590405a8.png" width="23%"/>
</p>
</br>
<p align="left">
...
> <b> 패키지구조 <b/>
> - Domain 중심으로 연관된 코드를 찾기 쉽게 설계 했습니다.   
</br>

> <b> 디자인패턴 <b/>
> - MVVM 패턴을 적용해 각 레이어간 의존도를 낮춰 유지보수가 용이하도록 만들었습니다.
</br>

> <b> 라이브러리 및 기술 <b/>
> - Paging3을 사용해 수백개의 부서 정보 아이템을 페이징해 메모리 효율을 높여 ANR을 방지했습니다.
> - CustomView로 반복되는 뷰 집합을 하나로 묶어 코드 가독성을 높이고, 유지보수가 용이하도록 만들었습니다.
> - Retrofit2를 사용해 백엔드 서버와 비동기 통신으로 데이터를 받아왔습니다. 
> - Coroutine을 사용해 비동기적으로 백그라운드 작업을 처리하고, 필요한 경우 ui의 데이터를 업데이트 해줬습니다.
> - Fragment를 배치해 액티비티 내의 위치정보, 지도찾기의 비즈니스 로직을 분리해 코드 결합도를 낮췄습니다.
> - Room DB로 수업 녹음 내부저장소 경로 등 서버 관리가 불필요한 정보를 로컬에 저장해 효율을 높였습니다.
> - 기타 : ViewPager, LiveData, DataBinding, AlarmManager .. etc
</br>

> <b> 외부 라이브러리 <b/>
> - Kakao Map에서 위도, 경도 계산을 통해 부서명으로 건물을 찾고, 현 위치에서 걸리는 시간을 계산했습니다. 
> - Google FCM으로 학내 실시간 크롤링 정보를 푸시 했습니다.
> - Googel Drive Api를 통해 수업 녹음 정보를 백업 가능하도록 해 안전성을 높였습니다. 
</br>
...
> - IDE : InteliJ
> - 데이터베이스 : MySQL 
> - 클라우드 환경 : AWS EC2, Route53
> - 라이브러리 및 기술 : Jsoup, FCM, Lombok ..etc 
> - Https 관련 : LetsEncrypt, 도메인 Gabia 
</br>
</br>
</br>
...
> - 만들때는 즐거웠지만 만들고나서 화면을 보니 너무 초라해서 아쉬웠습니다. 
...
> - 특히 액티비티에서 반복되는 뷰 관련 작업 코드들을 BindingAdapter로 처리했더라면 코드의 분리를 노릴 수 있었는데 활용이 아쉽습니다.  
> - 다음 프로젝트에서 MVVM 패턴을 구현 시, 바인딩어댑터를 적극적으로 활용할 것입니다 
...
> - DIFF_UTIL을 학습하지 않고 쓴 점([학습 노션 링크](https://unexpected-train-8e4.notion.site/DiffUtil-3c28b163a3784efcb4d308f11e06902a)) / 체계적이지 않은 구글 플랫폼 관리 / suspend 키워드의 통일되지 않은 사용 
> - 주석의 내용이 통일적이지 않은 점 + 안써도 될 부분도 굳이 쓴 점(~1.23 개선 완료)  / 유효성 체크 미비 / 초중반부의 깃헙커밋 메시지의 가벼움 / 정해지지 않은 테스트 과정 
> - String 리소스를 이용하지 않은 점<b>(1.19 개선 완료)</b> / 테스트시에만 쓰고 지운 로그들    
